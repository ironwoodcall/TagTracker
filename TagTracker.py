...
def valid_tag( tag:str ) ->bool:
    ...
    return bool(canonical_tag( tag ) in tkrconfig.all_tags)

def iprint(text:str, x:int=1) -> None:
    """Print the text, indented."""
    print(f"{tkrconfig.INDENT * x}{text}")
    ...
    try: # read saved stuff into dicts
        ...
        with open(filedir, 'r') as f:
            ...
            while line[0] != 'B': # while the first chr of each line isn't a header
                ...
                if not cells[0] in tkrconfig.all_tags or not valid_time(cells[1]):
...
def mode_stay(stays:list) -> Tuple[int,int]:
    ...
    round_stays = []
    for stay in stays:
        remainder = stay % tkrconfig.MODE_ROUND_TO_NEAREST
        mult = stay // tkrconfig.MODE_ROUND_TO_NEAREST
        if remainder > 4:
            mult += 1
        rounded = mult * tkrconfig.MODE_ROUND_TO_NEAREST
...
def show_stats():
    ...
    for tag in check_ins:
        if tag in tkrconfig.norm_tags:
            norm += 1
        elif tag in tkrconfig.over_tags:
            over += 1
    tot_in = norm + over
    ...
    if len(check_outs) > 0: # if any bikes have completed their stay, do stats
        ...
        for stay in all_stays: # count stays over/under x time
            if stay < tkrconfig.T_UNDER:  # Changed from <= to < so matches description.
                short_stays += 1
            elif stay <= tkrconfig.T_OVER:
                medium_stays += 1
            else:
                long_stays += 1
        hrs_under = f"{(tkrconfig.T_UNDER / 60):3.1f}" # times in hours for print clarity
        hrs_over = f"{(tkrconfig.T_OVER / 60):3.1f}"
        ...
        iprint(f"Mode stay:    {minutes_to_time_str(mode):>5} "
               f"by {count} bike(s)  [{tkrconfig.MODE_ROUND_TO_NEAREST} minute blocks]")
...
def delete_entry(target = False, which_to_del=False, confirm = False) -> None:
    ...
    if not(target in [False] + tkrconfig.all_tags or which_to_del in [False, 'b', 'o']
    ...
    if not target: # get target if unspecified
        ...
        target = input(f"(tag name) {tkrconfig.CURSOR}").lower()
    ...
    if not checked_in and not checked_out:
        ...
        if not which_to_del: # ask which to del if not specified
            ...
            which_to_del = input(f"(b/o) {tkrconfig.CURSOR}").lower()
        if which_to_del == 'b':
            if confirm == 'y': # pre-confirmation
                ...
                iprint("Are you sure you want to delete both events "
                       f"for {target}? (y/N)")
                sure = input(f"(y/N) {tkrconfig.CURSOR}").lower() == 'y'
            if sure:
...
def query_tag(target = False, do_printing = True) -> str:
    ...
    if not target: # only do dialog if no target passed
        ...
        target = input(f"(tag name) {tkrconfig.CURSOR}").lower()
    if target in tkrconfig.retired_tags:
...
def prompt_for_time(inp = False) -> bool or str:
    ...
    if not inp:
        ...
        inp = input(f"(HH:MM) {tkrconfig.CURSOR}").lower()
    if inp == 'now':
        return get_time()
    elif valid_time(inp):
        HH = inp[:2]
...
def edit_entry(target = False, in_or_out = False, new_time = False):
    ...
    if not target:
        ...
        target = input(f"(tag ID) {tkrconfig.CURSOR}").lower()
    elif not target in tkrconfig.all_tags:
        iprint(edit_syntax_message)
        return False
    if target in tkrconfig.all_tags:
        if target in check_ins:
            if not in_or_out:
                iprint("Do you want to change this bike's "
                       "check-(i)n or check-(o)ut time?")
                in_or_out = input(f"(i/o) {tkrconfig.CURSOR}").lower()
...
def tag_check(tag:str) -> None:
    ...
    """
    if not tag in tkrconfig.retired_tags:
        ...
        if checked_in:
            if checked_out:# if string is in checked_in AND in checked_out
                ...
                sure = input(f"(y/N) {tkrconfig.CURSOR}") == 'y'
                if sure:
...
def process_prompt(prompt:str) -> None:
    ...
    try:
        ...
        elif kwd in tkrconfig.statistics_kws:
            show_stats()
        elif kwd in tkrconfig.help_kws:
            print(tkrconfig.help_message)
        elif kwd in tkrconfig.audit_kws:
            show_audit()
        elif kwd in tkrconfig.edit_kws:
...
def main() -> None:
    ...
    prompt = input(f"\n\nEnter a tag or command {tkrconfig.CURSOR}").lower() # take input
...
print(f"TagTracker {tkrconfig.VERSION} by Julias Hocking")
DATE = get_date()
LOG_FILEPATH = f"logs/{tkrconfig.LOG_BASENAME}{DATE}.log"

if read_tags(): # only run main() if tags read successfully
    main()
else: # if read_tags() problem
    print(f"\n{tkrconfig.INDENT}Closing automatically in 30 seconds...")
    time.sleep(30)