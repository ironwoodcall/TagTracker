...
| `Hd` | Held — out of service just for today |

**Held vs. retired** — retired is forever (until deliberately
unretired); held resets to nothing every single morning, no matter what.

**Every day is a clean slate** — visits, notes, registrations, hours,
and held tags all start empty each day. The retired list and the tag
...
delete/hold/unhold, for a couple of minutes only; `redo` reverses an
`undo`.
...
| `hold <tag>` / `unhold <tag>` | Set aside for today / release |
...
5. **Held** — temporarily out of service, just for today.

A tag cycles freely between available → checked in → checked out → (in
again, with a new bike) all day long. Retired and held are the two
...
### Held: temporarily out of service, but not retired
...
- **A bike gets set aside mid-day.** Say it has no kickstand, so it ends
  ...
  handing it out for a new bike would be confusing. Mark that tag held
  so it doesn't get reused, and so it doesn't look like a mysterious gap
  later when someone's scanning the board.

Marking a tag held is always a judgment call *you* make — the tracker
never guesses at it on its own.

A tag can only be held if it's currently available or already checked
out today — not while a bike is actually checked in on it (check it out
first), and not if it's retired.

**Held tags reset every day.** Unlike retiring, holding is a
same-day-only decision — every morning starts with zero held tags. If
the same bike is *still* sitting in the lockup the next day, it needs to
be marked held again; the tracker won't remember it for you.

## Every day is a clean slate

Each day's check-ins, check-outs, notes, registrations, operating hours,
and held tags belong to that day alone — a new day starts every one of
...
delete, hold, or unhold, but only for a couple of minutes after you did
...
- **Tags held** — every tag currently marked held (only appears if at
  least one tag is held).
...
checked in, checked out, held, or retired even if that's not the
...
One quirk worth knowing: the held grid always reflects the *current*
moment, not the time you asked the audit report for. If you run
`audit 09:00` in the afternoon, the held section still shows who's held
*right now* — hold and unhold aren't logged with a time the way
check-ins and check-outs are.

### Other views

`query <tag>` answers "what's going on with this one tag today" —
every visit it's had, and whether it's held or retired. `recent` shows
